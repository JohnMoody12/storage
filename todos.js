...
app.post(
  ...
  catchError(async (req, res) => {
    ...
    let todoListUser = res.locals.username;
    ...
    if (!errors.isEmpty()) {
      ...
      let created = await res.locals.store.createTodoList(
        todoListTitle,
        todoListUser
      );
      if (!created) {
...
app.post(
  ...
  catchError(async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      let created = await res.locals.store.createTodo(
        +todoListId,
        todoTitle,
        todoListUser
      );
